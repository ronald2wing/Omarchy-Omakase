...
Panel {
  ...
  readonly property string stateDir: Quickshell.env("HOME") + "/.local/state/omakase"
  ...
  // name+"\u0000"+canonicalCity -> true set for entries that HAVE a city, and
  // normalized-name -> true set for entries whose city is empty — both rebuilt
  // whenever `saved` changes so isSaved() is O(1) instead of an O(saved) indexOf
  // scan per card. Null-prototype so a saved name like "constructor" can't
  ...
  // Total spot count across all cities, recomputed on each catalog change
  // (catalogVersion is a tracked dep below).
  readonly property int totalSpotCount: {
    root.catalogVersion;
    return CatalogUtils.totalSpotCount(root.catalog.spotsByCity);
  }

  readonly property string searchPlaceholder: "Search " + CatalogUtils.spotCountLabel(root.totalSpotCount) + "…"
  ...
  // (totalSpotCount, sortedCities) re-evaluate. Distinct from catalogToken, the
  ...
  function refreshSavedSet() {
    ...
    // city key on normalizedName + "\0" + canonicalCityKey, and entries whose
    // city is empty (legacy/unresolved) keyed by normalized name alone — the
    // name-only fallback mirroring findJournalIndex. isSaved() reads both, so
    // it stays O(1) and case-insensitive; the on-disk `saved` array still holds
    // the original casing unchanged.
    for (var i = 0; i < root.saved.length; i++) {
      var entry = root.saved[i];
      if (!entry || typeof entry !== "object") continue;
      var name = SpotUtils.normalizeName(entry.name);
      if (!name) continue;
      var city = JournalIdentity.canonicalCityKey(entry.city, root.cityAliases);
      if (city) byCity[name + "\u0000" + city] = true;
      else nameOnly[name] = true;
    }
  ...
  function isSaved(spotName, cityKey) {
    ...
    if (city && root.savedByCitySet[key + "\u0000" + city]) return true;
  ...
  // comparable name (`nameOf`) matches — the shared "normalize then scan for
  // membership" rule checkHoldRelease (spot/entry lists) and toggleSaved (the
  // plain string list) both used to hand-roll. Returns the first matching
  // index, or -1.
  ...
  function surpriseMe() {
    ...
    root.spotlight = true;
    // Opening Spotlight navigates away from the search field: dismiss any
    // lingering suggestions so the dropdown (and its overlay) never covers the
    // spotlight card.
    root.searchSuggestionsDismissed = true;
    // Opening Spotlight also navigates away from the filter sheet: close it so
    // it does not float over the spotlight card.
  ...
  function switchTab(tab) {
    root.tab = tab;
    // Entering a tab must not raise the suggestions dropdown: dismiss it, so a
    // leftover query in the destination view (still matching the focus the
    // field keeps across the switch) can't reopen it. Typing re-opens via the
    // query-change clears below.
    root.searchSuggestionsDismissed = true;
    // The sheet is a single root bool shared by all three FilterPanel
    // instances; without this it would follow the tab switch. Close it so the
    // destination tab's primary row is unobstructed.
    root.openSheet = false;
  ...
  // Dismiss any open popover — the filter sheet and the search-suggestions
  // dropdown — shared by the two dismiss TapHandlers (the overlay below the
  // pinned header and the header's own dismiss surface), so the outside-tap
  // behavior lives in exactly one place. The sheet's onOpenSheetChanged
  // clears openDropdown; the dropdown's searchSuggestionsDismissed is the
  // same flag handleSearchEscape sets.
  function dismissPopovers() {
    if (root.openSheet) root.openSheet = false;
    if (root.searchSuggestionsVisible) root.searchSuggestionsDismissed = true;
  }
  ...
  // KEEPING the radius — the Radius pill owns the radius itself, so it must not
  // route through selectLocationScope (which zeroes radius). The name
  // distinguishes it from FilterBar's clearLocationScope, which zeroes the
  // radius.
        ...
        Column {
          ...
          Row {
            ...
            TabChips {
              ...
              currentTab: root.spotlight ? "" : root.tab
