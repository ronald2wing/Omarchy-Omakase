...
// Total spot count across every city — the sum of each city array's length.
// A non-object input (a missing/empty catalog) counts 0, matching the widget's
// inline reduce (which defaults a falsy spotsByCity to {}).
function totalSpotCount(spotsByCity) {
  if (!spotsByCity || typeof spotsByCity !== "object") return 0;
  return Object.keys(spotsByCity).reduce(function (sum, city) {
    return sum + spotsByCity[city].length;
  }, 0);
}
