...
// ----- totalSpotCount -----
assert("totalSpotCount sums every city's array length",
  M.totalSpotCount({ nyc: [{}, {}], tokyo: [{}], la: [] }) === 3);
assert("totalSpotCount empty map", M.totalSpotCount({}) === 0);
assert("totalSpotCount non-object input counts 0",
  M.totalSpotCount(null) === 0 && M.totalSpotCount(undefined) === 0 && M.totalSpotCount("x") === 0);
...
// Two 31-bit rolling hashes over JSON.stringify plus the serialized length, so
// the result is "<length>:<hashA base36>:<hashB base36>" — pin that shape.
assert("catalogSpotsHash prefixes the serialized length and two base36 hashes",
  (function () {
    const spotsByCity = { a: [{}] };
    const hash = M.catalogSpotsHash(spotsByCity);
    const parts = hash.split(":");
    return parts.length === 3 &&
           parts[0] === String(JSON.stringify(spotsByCity).length) &&
           /^[0-9a-z]+$/.test(parts[1]) && /^[0-9a-z]+$/.test(parts[2]);
  })());
