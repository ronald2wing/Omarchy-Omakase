...
# Abort after this many consecutive hard 429s (search_yelp returning nil): a
# sustained rate-limit means the key/network is blocked, so fail loudly rather
# than hammering the API forever.
MAX_CONSECUTIVE_429 = 8
# Phase 1 caps the bulk "omakase" search at this offset (YELP_PAGE_SIZE pages),
# the upper bound of the window worth paying for. YELP_PAGE_SIZE lives in
# yelp_client.rb so collect/refresh can't drift.
YELP_SEARCH_MAX_OFFSET = 250
# Phase 2 exact-name search result count — a single spot match is all that's
# needed, so a small limit keeps the call cheap.
...
SEARCH_TRIGGER_FIELDS = %w[yelp_url yelp_rating address state display_address].freeze
...
# isolates every file read/write to a temp dir, and `sleeper` lets tests
# suppress the real rate-limit pauses. Defaults keep the CLI path identical.
def refresh_yelp_fields(search: method(:search_yelp), data_dir: DATA_DIR,
                        city_arg: nil, sleeper: ->(secs) { sleep(secs) })
  ...
  search_counted = lambda do |params|
    ...
    if businesses.nil?
      consecutive_429 += 1
      abort "\nAbort: consecutive 429s." if consecutive_429 >= MAX_CONSECUTIVE_429
      return nil
    end
    consecutive_429 = 0
    calls += 1
  ...
  files.each do |filename|
    ...
    write_atomic(path, entries) if bulk_changed
  ...
  { filled: filled, calls: calls }
end

if __FILE__ == $PROGRAM_NAME
  refresh_yelp_fields(city_arg: ARGV[0])
end
