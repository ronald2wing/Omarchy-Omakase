...
QtObject {
  ...
  function runChunked(opts, token) {
    ...
    if (token !== opts.getToken()) return;
    if (more) {
      // Closure, not a bare method reference — see AGENTS.md (chunked-walk reschedule).
  ...
  // { processed, more }. The advance is shared by every walk from a single exit
  // path; only the per-spot body and per-city hook differ.
  function buildSpotStep(opts) {
    return function (ctx) {
      ...
      // One exit path for the city-boundary advance: it covers both the
      // in-loop exhaustion and the empty-city case (spot 0 with no spots), so
      // `onCityDone` fires exactly once per city.
      if (ctx.spot >= spots.length) {
        if (opts.onCityDone) opts.onCityDone(ctx, cityKey);
        ctx.spot = 0;
        ctx.city++;
      }
      return { processed: processed, more: ctx.city < cities.length };
    };
  }

  // Shared spot-walk wiring: wraps buildSpotStep into runChunked's contract so
