...
def select_city_files(files, city_arg)
  ...
  if selected.empty?
    warn_unknown_city(files, city_arg)
    exit 1
  end
  selected
end

# Warn that `city_arg` has no matching data file, listing the available city
# keys. Shared by select_city_files (single-city) and sort_data.rb's multi-city
# validation so the message stays byte-identical. Only prints; never exits.
def warn_unknown_city(files, city_arg)
