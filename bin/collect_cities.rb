...
def collect_cities(search: method(:search_yelp), data_dir: DATA_DIR,
  ...
  # An aborted city is a failed run for a shell/CI caller, so return the abort
  # count alongside the entry total for the CLI block below.
  { total: total, incomplete: incomplete_cities }
end
...
if __FILE__ == $PROGRAM_NAME
  ...
  result = collect_cities(dry_run: flags.dry_run)
  exit 1 if result[:incomplete].positive?
end
