...
Column {
  ...
  Column {
    ...
    Item {
      width: parent.width
      // Notes content height: the text plus its vertical padding, floored at
      // the minimum box height. Shared by the wrapper height (which caps it and
      // lets the box scroll) and the Flickable's contentHeight, so the two can
      // never drift apart.
      readonly property real notesContentHeight: Math.max(notesEdit.implicitHeight + Style.space(16), Style.space(56))
      ...
      height: Math.min(notesContentHeight, Style.space(120))
      Flickable {
        anchors.fill: parent
        clip: true
        contentWidth: width
        ...
        TextEdit {
          ...
          onTextChanged: {
            ...
            ratingEditor.panel.notesEdited(ratingEditor.spotName, text, ratingEditor.cityKey);
          }
    ...
    // Remove rating spans the full editor width now that Save is gone. It sits
    // 12px below the notes box — the Column's 6px spacing plus a 6px top inset
    // (formerly a wrapping Row's topPadding). A Positioner ignores anchors on
    // its children, so the inset can't be a top margin on the button itself;
    // the button lives in a transparent container 6px taller instead.
    Rectangle {
      visible: hasRating
      width: parent.width
      height: Style.space(40) + Style.space(6)
      color: "transparent"
      Rectangle {
        ...
        MouseArea { anchors.fill: parent; cursorShape: Qt.PointingHandCursor
          onClicked: {
            ...
            ratingEditor.panel.unrateSpot(spotName, cityKey);
