...
def first_display_address(entry)
  ...
  display.is_a?(Array) && display.find { |el| present_string?(el) }
end
...
def derive_address(entry)
  ...
  return nb if present_string?(nb) && address_like?(nb)
...
def clean_neighborhood(nb)
  return nil unless present_string?(nb)
...
def normalize_entry(entry)
  ...
  pairs.delete_if { |_, value| blank_value?(value) }
  pairs.to_h
end

private

# True when `value` is a non-blank string: nil, empty, and whitespace-only values
# are false; any other value is true. Distinct from `blank_value?` (pipeline_common),
# which does not strip whitespace.
...
if __FILE__ == $PROGRAM_NAME
  ...
  files.each do |path|
    ...
    entries.each do |entry|
      original_nb = entry['neighborhood']
      has_nb = present_string?(original_nb)
      kept_nb = clean_neighborhood(original_nb)
