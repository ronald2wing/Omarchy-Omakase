...
The panel has three tabs (the header chips show the Wishlist and Journal
counts):
...
- **Discount** — a happy-hour deal (a time and a discount are both listed).
- **Omakase** — a set menu (a course count and a price are both listed).
...
only. The stars on a collapsed card row quick-rate without expanding, and the
collapsed card's **Phone** cell dials (or copies the number) in one click.
