...
# The Nominatim query string for one spot — the spot's street address
# disambiguated by the city label. Shared by the dry-run call count and the
# geocode loop so the counted query and the issued query are the identical
# string, and therefore memoize together.
def geocode_query(entry, label)
...
def geocode_file(path, city_key, search_provider:, done:, progress_file:, query_cache:, dry_run:, sleeper:, failures:, state:)
  ...
  geocode_work_items(placeholders, city_key, done).each do |entry, lat, lon, key|
    query = geocode_query(entry, label)
