...
// State directory suffix and the two state-file names the Service owns
// (state.json / journal.json). Single source for the path literals both entry
// points build; each file still reads its own HOME (Quickshell.env is not
// callable from a JS module). wishlist.json is deliberately absent — the bar
// widget alone owns it. `var` (not `const`) so QML's import qualifier exposes
// them as SpotUtils.NAME.
var STATE_DIR_SUFFIX = "/.local/state/omakase";
var STATE_FILE = "state.json";
var JOURNAL_FILE = "journal.json";
...
// True for a non-null, non-array object — the shared "is a plain object"
// predicate every shape guard uses to reject null, primitives, and arrays
// alike (the same non-object input that would otherwise throw on a field read).
