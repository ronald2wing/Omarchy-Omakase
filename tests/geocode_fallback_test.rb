...
class GeocodeFallbackTest < Minitest::Test
  ...
  def test_missing_coordinates_group_as_one_coarse_placeholder
    ...
    # is true (decimal_places(nil) is 0), so that group survives placeholder_groups'
    # size>1 + coarse filter and its entries are geocoded from their address.
    entries = [{ 'name' => 'A', 'address' => '1 Main St' },
               { 'name' => 'B', 'address' => '2 Main St' }]
    groups = placeholder_groups(entries)
  ...
  def test_default_search_provider_is_a_one_argument_callable
    ...
    # the one argument the call site passes by invoking it with the real search
    # stubbed, so no network call is made.
    main = TOPLEVEL_BINDING.receiver
    main.define_singleton_method(:nominatim_search) { |_query, **_kwargs| [] }
    assert_equal [], NOMINATIM_SEARCH_PROVIDER.call('some query'),
                 'the default provider must accept one argument without raising'
  ensure
    main.singleton_class.remove_method(:nominatim_search) if main.singleton_class.method_defined?(:nominatim_search)
  end
end
