...
class FoursquareBatchTest < Minitest::Test
  ...
  # Mirrors the shared stub_foursquare helper (test_helper.rb), one level lower:
  # the tests below pass `http:` so the real foursquare_batch runs against a
  # fake HTTP transport, instead of stubbing the batch wholesale. The `http:`
  # callable receives the response-sink block http_request normally yields to,
