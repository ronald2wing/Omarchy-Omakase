...
# value (otherwise `[].to_s == "[]"` would mark the field as set). Used by
# fill_if_absent! (a blank Yelp value is not written) and by
# normalize_locations.rb's normalize_entry (the final blank-value sweep).
...
# True when an entry carries a non-blank street `address` — nil, "", and
# whitespace-only all count as absent. Shared by geocode_fallback.rb
# (geocode_work_items + its address-less reporting loop, so the "geocodable"
# and "reported as no-address" sets are exact complements) and
# reverse_geocode_addresses.rb (run_validation's sample filter +
# reverse_work_items' already-addressed skip).
