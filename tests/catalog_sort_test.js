...
// ----- decoratedSort (reached via sortBySpec) -----
const decorateItems = [
  { name: "Beta", rating: 4.0 },
  { name: "Alpha", rating: 4.5 },
  { name: "Gamma", rating: 3.5 }
];
const ratingNoTieSpec = {
  rating: { extract: (s) => s.rating }
};
assert("decoratedSort rating desc",
  JSON.stringify(M.sortBySpec(decorateItems, "rating", false, ratingNoTieSpec).map((s) => s.name)) === '["Alpha","Beta","Gamma"]');
const distanceItems = [
  { name: "Far", km: 15 },
  { name: "Near", km: 5 },
  { name: "Mid", km: 10 }
];
const distanceSpec = { distance: { extract: (s) => s.km } };
assert("decoratedSort distance asc",
  JSON.stringify(M.sortBySpec(distanceItems, "distance", false, distanceSpec).map((s) => s.name)) === '["Near","Mid","Far"]');
assert("decoratedSort distance reversed flips",
  JSON.stringify(M.sortBySpec(distanceItems, "distance", true, distanceSpec).map((s) => s.name)) === '["Far","Mid","Near"]');
