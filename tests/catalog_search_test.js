...
assert("matchClass empty query returns input via rankSpots",
  (function () {
    const spots = [{ name: "sushi", neighborhood: "" }];
    return M.rankSpots(spots, "") === spots;
  })());
assert("matchClass blank name does not throw via rankSpots",
  JSON.stringify(M.rankSpots([{ name: null, neighborhood: "" }], "sushi")) === '[]');
...
assert("buildLocationBundle assembles the nine-field shape",
  (function () {
    ...
    return JSON.stringify(Object.keys(b)) === '["cities","names","counts","byCity","foldedNames","foldedKeys","states","countries","stateDistances"]' &&
