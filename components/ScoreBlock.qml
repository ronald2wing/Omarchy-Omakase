...
Item {
  ...
  // Unrated spots show the Yelp stars muted, no number. Hovering a star
  // previews the rating a click would set (stars 1..N fill) and grows the row,
  // so it reads unmistakably as "yours to set" (the pointer cursor signals the
  // same); the resting appearance still shows the committed rating — or Yelp's
  // at 55% when unrated, "this is Yelp's rating, not yours yet".
  // Star-row design tokens, so the hover scales and the muted unrated opacity
  // read by name instead of bare literals.
  readonly property real starPreviewScale: 1.2
  readonly property real starHoverScale: 1.12
  readonly property real unratedStarOpacity: 0.55
  Row {
    id: starsRow
    ...
    StarRating {
      id: stars
      panel: block.panel
      value: block.usingYelpStars ? block.yelpRating : block.starValue
      interactive: block.starsInteractive
      opacity: block.usingYelpStars ? (stars.rowHovered ? 1.0 : block.unratedStarOpacity) : 1.0
      // The 55% → 100% brightening alone is subtle at this size, so hover adds
      // a scale: previewing a star grows the row a little more than the
      // whole-row hover cue on a non-interactive unrated row (starHoverScale).
      // The resting appearance is untouched.
      scale: stars.hoveredStar > 0 ? block.starPreviewScale : (block.usingYelpStars && stars.rowHovered ? block.starHoverScale : 1.0)
  ...
  Tooltip {
    ...
    visible: stars.rowHovered && block.starValue > 0 && block.yelpRating > 0
