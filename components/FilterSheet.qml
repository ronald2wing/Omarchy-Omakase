...
Rectangle {
  ...
  readonly property var locationRowSpecs: [
    { label: "City",         placeholder: "All cities",        dropdownName: CatalogUtils.SCOPE_CITY },
    { label: "State",        placeholder: "All states",        dropdownName: CatalogUtils.SCOPE_STATE },
    { label: "Country",      placeholder: "All countries",     dropdownName: CatalogUtils.SCOPE_COUNTRY },
    { label: "Neighborhood", placeholder: "All neighborhoods", dropdownName: CatalogUtils.SCOPE_NEIGHBORHOOD }
  ]
  ...
  function pickLocation(dropdownName, key) {
    if (dropdownName === CatalogUtils.SCOPE_CITY) sheet.panel.selectCity(key);
    else if (dropdownName === CatalogUtils.SCOPE_STATE) sheet.panel.selectState(key);
    else if (dropdownName === CatalogUtils.SCOPE_COUNTRY) sheet.panel.selectCountry(key);
    else if (dropdownName === CatalogUtils.SCOPE_NEIGHBORHOOD) sheet.panel.selectNeighborhood(key, sheet.panel.cityKey);
  }
    ...
    Repeater {
      ...
      delegate: FilterSheetLocationRow {
        ...
        value: {
          ...
          if (name === CatalogUtils.SCOPE_CITY) return sheet.panel.cityLabel(sheet.panel.cityKey);
          if (name === CatalogUtils.SCOPE_STATE) return sheet.panel.stateFilter !== "" ? CatalogUtils.stateNameFor(sheet.panel.stateFilter) : "";
          if (name === CatalogUtils.SCOPE_COUNTRY) return sheet.panel.countryFilter;
        ...
        items: {
          var name = modelData.dropdownName;
          if (name === CatalogUtils.SCOPE_CITY) return sheet.panel.cityFilterItems;
          if (name === CatalogUtils.SCOPE_STATE) return sheet.panel.stateFilterItems;
          if (name === CatalogUtils.SCOPE_COUNTRY) return sheet.panel.countryFilterItems;
        ...
        opensOnClick: modelData.dropdownName === CatalogUtils.SCOPE_CITY
        dropdownAllowed: {
          var name = modelData.dropdownName;
          if (name === CatalogUtils.SCOPE_STATE) return sheet.panel.stateFilter === "";
          if (name === CatalogUtils.SCOPE_COUNTRY) return sheet.panel.countryFilter === "";
          return true;
        }
        // Neighborhood only when the selected city has neighborhoods worth
        // filtering by.
        visible: modelData.dropdownName !== CatalogUtils.SCOPE_NEIGHBORHOOD || sheet.panel.hasNeighborhoodFilter
